...
function deterministicReply(userMessage: string, metrics: any): string {
  ...
  if (q.includes('leak') || q.includes('erreur') || q.includes('faiblesse')) {
    return `🎯 **Ton leak prioritaire**\n\nSur ${total} trades, je chercherais d'abord le facteur qui dégrade le plus ton expectancy, pas simplement ton Win Rate.\n\n• Win Rate : ${wr}\n• Expectancy : ${exp}\n• PnL total : ${pnl}\n\nCompare tes pertes par setup, session et actif. Le groupe avec expectancy négative et un échantillon suffisant est le premier candidat à corriger.`;
  }
  if (q.includes('setup') || q.includes('meilleur')) {
    const edges = Array.isArray(metrics?.edges) && metrics.edges.length ? metrics.edges.slice(0,3).join(' ; ') : 'pas encore assez de groupes identifiés';
    return `📈 **Tes setups**\n\nD'après les données actuellement transmises au coach :\n\n${edges}\n\nNe choisis pas un setup uniquement parce qu'il affiche le meilleur Win Rate : vérifie aussi son nombre de trades, son PnL et son expectancy.`;
  }
  if (q.includes('discipline') || q.includes('risque') || q.includes('risk')) {
    return `🧠 **Discipline & risque**\n\nJe regarderais trois choses : respect du Stop Loss, stabilité de la taille de position et comportement après une perte.\n\nAvec ${total} trades enregistrés, évite de conclure trop vite sur une habitude avec seulement quelques observations. Ton objectif est de rendre le risque reproductible, pas seulement de maximiser le résultat d'un trade.`;
  }
  ...
  return `🎯 **Analyse de ta question**\n\nTu me demandes : « ${String(userMessage).trim()} »\n\nJe base cette réponse sur ${total} trades enregistrés. Ton Win Rate est ${wr}, ton expectancy est ${exp} et ton PnL total est ${pnl}.\n\nPour répondre précisément, je vais relier ta question à ces métriques plutôt que de répéter un diagnostic générique.`;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // If a Gemini key is configured in Vercel, use it. Otherwise the deterministic
  // context-aware response below guarantees that different questions never get
  // the same generic fallback response.
