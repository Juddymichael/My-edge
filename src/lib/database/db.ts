...
export class ThunderEdgeDatabase extends Dexie {
  ...
  async ensureSettings(): Promise<UserSettings> {
    ...
    if (!existing) { await this.settings.put(DEFAULT_USER_SETTINGS); return DEFAULT_USER_SETTINGS; }
    const migrated: UserSettings = {
      ...DEFAULT_USER_SETTINGS,
      ...existing,
      sessionSettings: { ...DEFAULT_USER_SETTINGS.sessionSettings, ...(existing.sessionSettings || {}) },
      riskPatternSettings: { ...DEFAULT_USER_SETTINGS.riskPatternSettings, ...(existing.riskPatternSettings || {}) },
    };
    const changed = JSON.stringify(migrated) !== JSON.stringify(existing);
    if (changed) await this.settings.put(migrated);
    return migrated;
  }
  async ensureDefaultSetups(): Promise<Setup[]> { const count = await this.setups.count(); if (count === 0) { await this.setups.bulkPut(DEFAULT_SETUPS); return DEFAULT_SETUPS; } return await this.setups.toArray(); }
}
