...
export class ExcelTradeParser implements ITradeParser {
  ...
  async parse(rawInput: string | ArrayBuffer): Promise<ParseResult> {
    const workbook = typeof rawInput === 'string'
      ? XLSX.read(rawInput, { type: 'binary', cellDates: true, cellNF: true })
      : XLSX.read(new Uint8Array(rawInput), { type: 'array', cellDates: true, cellNF: true });
    ...
    const rawMatrix: unknown[][] = XLSX.utils.sheet_to_json(worksheet, {
      header: 1, raw: false, dateNF: 'yyyy-mm-dd hh:mm:ss', defval: '', blankrows: true,
    });
    ...
    if (!isTradeHeader(headers)) {
      return { fileType: 'XLSX', totalRawRows: 0, rows: [], metadata: { sheetName: targetSheetName, headers, ignoredRows: rawMatrix.length - 1 } };
    }
    ...
    for (let i = headerRowIndex + 1; i < rawMatrix.length; i++) {
      ...
      if (symbolIndex >= 0 && !rowHasSymbol(values, symbolIndex)) { ignoredRows++; continue; }
    ...
    return {
      fileType: 'XLSX', totalRawRows: rows.length, rows,
      metadata: { sheetName: targetSheetName, totalSheets: workbook.SheetNames.length, headers, ignoredRows, invalidRows, sourceTotalPnl },
    };
  }
