...
export class CsvTradeParser implements ITradeParser {
  ...
  async parse(rawInput: string | ArrayBuffer, options?: Record<string, unknown>): Promise<ParseResult> {
    ...
    const nonEmpty = lines.map((line) => ({ line })).filter(({ line }) => line.trim().length > 0);
    ...
    const compactLines = nonEmpty.map((x) => x.line);
    const headerRowIndex = this.findHeaderRowIndex(compactLines, delimiter);
    const headers = this.parseCsvLine(compactLines[headerRowIndex] || '', delimiter).map((h) => h.trim().toLowerCase());
    ...
    for (let i = headerRowIndex + 1; i < compactLines.length; i++) {
      ...
      const symbolPresent = symbolIndex >= 0 && rowHasSymbol(cells, symbolIndex);
      if (!symbolPresent && pnlIndex >= 0) {
        const onlyPnl = cells.every((cell, index) => index === pnlIndex || !cell.trim());
        const numericPnl = normalizeNumber(cells[pnlIndex]);
      ...
      headers.forEach((header, colIdx) => { if (header) rowData[header] = cells[colIdx] !== undefined ? cells[colIdx].trim() : ''; });
  ...
  private detectDelimiter(sampleLines: string[]): string {
    const counts = sampleLines.reduce((acc, line) => { acc.comma += (line.match(/,/g) || []).length; acc.semi += (line.match(/;/g) || []).length; acc.tab += (line.match(/\t/g) || []).length; acc.pipe += (line.match(/\|/g) || []).length; return acc; }, { comma: 0, semi: 0, tab: 0, pipe: 0 });
  ...
  private findHeaderRowIndex(lines: string[], delimiter: string): number {
    for (let i = 0; i < Math.min(lines.length, 30); i++) if (isTradeHeader(this.parseCsvLine(lines[i], delimiter).map((c) => c.trim().toLowerCase()))) return i;
    return 0;
  }

  private parseCsvLine(line: string, delimiter: string): string[] {
    const result: string[] = []; let current = ''; let inQuotes = false;
    for (let i = 0; i < line.length; i++) { const char = line[i]; if (char === '"') { if (inQuotes && line[i + 1] === '"') { current += '"'; i++; } else inQuotes = !inQuotes; } else if (char === delimiter && !inQuotes) { result.push(current); current = ''; } else current += char; }
