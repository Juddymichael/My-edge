...
export class CsvTradeParser implements ITradeParser {
  ...
  async parse(rawInput: string | ArrayBuffer, options?: Record<string, unknown>): Promise<ParseResult> {
    ...
    if(!isTradeHeader(headers))return{fileType:'CSV',totalRawRows:0,rows:[],metadata:{delimiter,headerRowIndex,headers,ignoredRows:nonEmpty.length-1,invalidRows:0,sourceTotalPnl:null},sourceTotalPnl:null};
    ...
    return{fileType:'CSV',totalRawRows:rows.length,rows,sourceTotalPnl,metadata:{delimiter,headerRowIndex,headers,ignoredRows,invalidRows,sourceTotalPnl}};
  }
