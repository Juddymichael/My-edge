...
import { BarChart3, BookOpen, Calendar, FileUp, ListFilter, MoreHorizontal, Settings, Wallet, X, Zap, Sparkles } from 'lucide-react';
...
export const MobileNavigation: React.FC<MobileNavigationProps> = ({ activeTab, setActiveTab, settings, tradeCount, transactionCount }) => {
  ...
  return <>
    <AnimatePresence>
      {open && <div className="fixed inset-0 z-[60] md:hidden flex items-end">
    ...
    <nav className={`md:hidden fixed bottom-0 left-0 right-0 z-[55] border-t px-1 pt-1 pb-[calc(.25rem+env(safe-area-inset-bottom))] ${light ? 'bg-white/95 border-purple-200 text-slate-600' : 'bg-[#0B0F14]/95 border-[#252E38] text-[#8B96A3]'} backdrop-blur-xl`}>
      <div className="grid grid-cols-5 items-center">
        <button onClick={() => go('dashboard')} className={`relative flex flex-col items-center gap-0.5 py-1.5 rounded-xl ${activeTab === 'dashboard' ? (light ? 'text-violet-600' : 'text-[#f75605]') : ''}`}><div className={`w-7 h-0.5 rounded-full mb-0.5 ${activeTab === 'dashboard' ? (light ? 'bg-violet-600' : 'bg-[#f75605]') : 'bg-transparent'}`} /><ListFilter className="hidden" /><BarChart3 className="w-5 h-5" /><span className="text-[10px] font-bold">Accueil</span></button>
