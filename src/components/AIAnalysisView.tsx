...
import { AlertTriangle, BarChart3, CheckCircle2, ChevronRight, CircleHelp, Clock3, Lightbulb, Loader2, Plus, Sparkles, Target, TrendingUp, Zap } from 'lucide-react';
...
import { getHoldingTimeBucket, getHoldingMinutes, getTradeSession } from '../lib/tradingSession';
...
function aggregate(trades: Trade[], key: (t:Trade)=>string) {
  const map = new Map<string, Trade[]>();
  trades.filter(t=>t.status !== 'OPEN' && typeof t.netPnL === 'number').forEach(t => { const k=key(t); if(k) map.set(k,[...(map.get(k)||[]),t]); });
  return [...map.entries()].map(([name,ts])=>({ name, pnl: ts.reduce((s,t)=>s+(t.netPnL||0),0)/ts.length, winRate: ts.filter(t=>(t.netPnL||0)>0).length/ts.length*100, count:ts.length })).sort((a,b)=>a.name.localeCompare(b.name));
}
...
function RecommendationCard({ rec, onStatus, onApply, onView }: {rec:Recommendation;onStatus:(id:string,status:Recommendation['status'])=>void;onApply:(rec:Recommendation)=>void;onView:(rec:Recommendation)=>void}) {
  const priorityClass=rec.priority==='Élevée'?'text-rose-300 bg-rose-500/10 border-rose-500/20':rec.priority==='Moyenne'?'text-fuchsia-300 bg-fuchsia-500/10 border-fuchsia-500/20':'text-slate-300 bg-slate-700/30 border-slate-600';
  ...
  return <div className={`${card} p-5`}>
    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
      ...
      <div className="flex items-center gap-3 shrink-0"><div className="text-right"><div className="text-[9px] uppercase tracking-wider text-slate-500">Coût observé</div><div className="text-sm font-black text-rose-400">-{rec.estimatedCost.toFixed(0)} $</div></div><div className="w-14 h-14 rounded-full border border-fuchsia-500/20 flex items-center justify-center text-xs font-bold text-white">{Math.min(100,Math.max(0,Math.round(rec.impact)))}%</div></div>
    </div>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 mt-5">
      ...
      <div className="flex gap-2"><button onClick={()=>onView(rec)} className="px-3 py-2 rounded-xl border border-slate-700 text-xs font-bold text-slate-300 hover:border-violet-500/50">Voir le plan d'action <ChevronRight className="inline w-3.5 h-3.5"/></button><button onClick={()=>onApply(rec)} className="px-3 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white text-xs font-bold"><Plus className="inline w-3.5 h-3.5 mr-1"/>Appliquer à mon plan</button></div>
    </div>
  </div>;
}

const fallbackRecommendation = (alert:any, cost:number, currency:string, i:number): Recommendation => ({
  id:`local-${alert.id}-${i}`,
  title:alert.title,
  description:alert.explanation || 'Ce pattern a été détecté automatiquement dans vos trades. Réduisez son occurrence et mesurez son évolution sur les prochains trades.',
  priority:alert.type==='REVENGE_TRADING'||alert.type==='LOSS_STREAK'?'Élevée':'Moyenne',
  impact:cost>0?Math.min(100,Math.round(cost*.6)):25,
  effort:alert.type==='REVENGE_TRADING'?3:5,
  quickFixes:alert.type==='REVENGE_TRADING'?['Ajouter une pause après une perte avant toute nouvelle entrée.','Respecter la taille de risque prévue avant l’entrée.','Revoir le trade perdant avant de reprendre.']:alert.type==='LOSS_STREAK'?['Après plusieurs pertes, réduire temporairement la taille.','Faire une pause et vérifier les règles du setup.','Reprendre uniquement sur un signal conforme.']:['Documenter le pattern avant le prochain trade.','Définir une règle simple pour éviter sa répétition.','Suivre son coût sur les 20 prochains trades.'],
  category:'Pattern local',estimatedCost:cost,status:'Nouveau'
});
...
export const AIAnalysisView: React.FC<{trades:Trade[];setups:Setup[];settings:UserSettings;currency:string;onNotify?:(type:'success'|'error',message:string)=>void}> = ({trades,setups,settings,currency,onNotify}) => {
  ...
  const chartData=useMemo(()=>{
    const sessions=aggregate(closed,t=>getTradeSession(t));
    const duration=aggregate(closed,t=>getHoldingTimeBucket(getHoldingMinutes(t))||'Durée inconnue');
    const days=aggregate(closed,t=>{const d=new Date(t.closedAt||t.openedAt);return ['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'][d.getDay()];});
    const hours=aggregate(closed,t=>{const d=new Date(t.closedAt||t.openedAt);const utcPlus3=(d.getUTCHours()+3)%24;return `${String(utcPlus3).padStart(2,'0')}h`;});
    return {sessions,duration,days,hours};
  },[closed]);
  const baseRecommendations=useMemo(()=>activeAlerts.map((a,i)=>{const related=trades.filter(t=>a.relatedTradeIds.includes(t.id));const cost=Math.abs(related.filter(t=>(t.netPnL||0)<0).reduce((s,t)=>s+(t.netPnL||0),0));return fallbackRecommendation(a,cost,currency,i);}),[activeAlerts,trades,currency]);
  const displayedRecommendations=recommendations.length?recommendations:baseRecommendations;
  const runAnalysis=async()=>{setLoadingAnalysis(true);setAnalysisError(null);try{const r=await fetch('/api/ai-analysis',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'analysis',context})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Analyse indisponible');setAnalysis({summary:j.data.summary,keyPoints:Array.isArray(j.data.keyPoints)?j.data.keyPoints.slice(0,4):[],confidenceScore:Number(j.data.confidenceScore)||0});}catch(e){setAnalysisError(e instanceof Error?e.message:'Analyse indisponible');}finally{setLoadingAnalysis(false);}};
  const runRecommendations=async()=>{setRecommendations(baseRecommendations);setLoadingRecommendations(true);setRecommendationError(null);try{const patternCosts=activeAlerts.map(a=>{const related=trades.filter(t=>a.relatedTradeIds.includes(t.id));const cost=Math.abs(related.filter(t=>(t.netPnL||0)<0).reduce((s,t)=>s+(t.netPnL||0),0));return {...a,estimatedCost:cost};});const r=await fetch('/api/ai-analysis',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'recommendations',context:{summary:context.summary,alerts:patternCosts,setups:context.setups,sessions:context.sessions,pairs:context.pairs}})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Recommandations indisponibles');const ai=Array.isArray(j.data?.recommendations)?j.data.recommendations:[];if(ai.length){setRecommendations(ai.slice(0,8).map((x:any,i:number)=>{const base=baseRecommendations[i];const cost=Number(patternCosts[i]?.estimatedCost)||base?.estimatedCost||0;return {title:x.title||base?.title||'Recommandation',description:x.description||base?.description||'Action recommandée à partir des données locales.',priority:['Élevée','Moyenne','Faible'].includes(x.priority)?x.priority:(base?.priority||'Moyenne'),impact:Number(x.impact)||base?.impact||25,effort:Math.min(10,Math.max(1,Math.round(Number(x.effort)||base?.effort||5))),quickFixes:Array.isArray(x.quickFixes)&&x.quickFixes.length?x.quickFixes:(base?.quickFixes||['Appliquer une règle simple et mesurer son effet.']),category:x.category||base?.category||'Analyse IA',estimatedCost:cost,status:base?.status||'Nouveau',id:base?.id||`ai-${i}`};}));}}catch(e){setRecommendationError(e instanceof Error?e.message:'Recommandations indisponibles');}finally{setLoadingRecommendations(false);}};
  ...
  const optimizationScore=displayedRecommendations.length?Math.min(100,Math.max(20,Math.round(100-displayedRecommendations.filter(r=>r.priority==='Élevée').length*20)):100;
  return <div id="view-ai-analysis" className="space-y-6 font-sans text-slate-100">
    <div><div className="flex items-center gap-3"><div className="p-2.5 rounded-2xl bg-gradient-to-br from-violet-600/20 to-fuchsia-500/20 border border-violet-500/20"><Sparkles className="w-5 h-5 text-fuchsia-300"/></div><div><h1 className="text-2xl font-black tracking-tight text-white">Analyse IA</h1><p className="text-xs text-slate-500 mt-1">Audit structuré de votre performance, sans chat libre.</p></div></div>
      <div className="mt-5 p-1 rounded-2xl bg-[#111118] border border-slate-800 flex gap-1 w-full sm:w-fit">{[['analysis','Analyse'],['charts','Graphiques'],['recommendations','Recommandations']].map(([id,label])=><button key={id} onClick={()=>{setTab(id as Tab);if(id==='analysis'&&!analysis&&!loadingAnalysis)void runAnalysis();if(id==='recommendations'&&!loadingRecommendations&&!recommendations.length)void runRecommendations();}} className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all ${tab===id?'bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white shadow-lg shadow-fuchsia-500/10':'text-slate-500 hover:text-white'}`}>{label}</button>)}</div>
    </div>
    {tab==='analysis'&&<div className="space-y-5">
      {analysisError&&<div className="p-4 rounded-2xl border border-fuchsia-500/25 bg-fuchsia-500/5 text-fuchsia-200 text-xs flex flex-wrap items-center gap-3"><AlertTriangle className="w-4 h-4 shrink-0"/><span className="flex-1">{analysisError.includes('GEMINI_HIGH_DEMAND')?'Gemini est temporairement surchargé, réessaie dans quelques instants':analysisError}</span><button onClick={()=>void runAnalysis()} className="px-3 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white font-bold">Réessayer</button></div>}
      {!analysis&&!loadingAnalysis&&!analysisError&&<button onClick={()=>void runAnalysis()} className={`${card} w-full p-8 text-left hover:border-violet-500/40 transition-colors`}><div className="flex items-center gap-3"><Lightbulb className="w-5 h-5 text-fuchsia-300"/><div><div className="text-sm font-bold text-white">Générer mon résumé exécutif IA</div><div className="text-xs text-slate-500 mt-1">Un appel Gemini unique basé sur vos statistiques réelles.</div></div><ChevronRight className="w-5 h-5 ml-auto text-slate-600"/></div></button>}
      {loadingAnalysis&&!analysis&&<div className={`${card} p-6`}><div className="flex items-center gap-2 text-xs text-slate-400"><Loader2 className="w-4 h-4 animate-spin text-fuchsia-400"/>Génération du résumé IA…</div><div className="mt-4 h-3 w-full bg-slate-800 rounded animate-pulse"/><div className="mt-2 h-3 w-4/5 bg-slate-800 rounded animate-pulse"/></div>}
    ...
    {tab==='charts'&&<div className="grid lg:grid-cols-2 gap-5"><PerformanceChart title="Performance par session" data={chartData.sessions} info="La session est calculée automatiquement en UTC+3 à partir de l'heure d'ouverture si le trade n'en contient pas une manuelle."/><PerformanceChart title="Performance par unité de temps" data={chartData.duration} info="Ici, unité de temps signifie durée moyenne de détention : les trades sont regroupés par tranches de durée entre ouverture et clôture."/><PerformanceChart title="Performance par jour de la semaine" data={chartData.days} info="Mesure la performance moyenne et le win rate selon le jour de clôture."/><PerformanceChart title="Performance par heure de la journée" data={chartData.hours} info="Heure convertie en UTC+3 (Madagascar) pour tous les trades afin de comparer les mêmes horaires." /></div>}
    {tab==='recommendations'&&<div className="space-y-5">
      {recommendationError&&<div className="p-4 rounded-2xl border border-fuchsia-500/25 bg-fuchsia-500/5 text-fuchsia-200 text-xs flex flex-wrap items-center gap-3"><AlertTriangle className="w-4 h-4 shrink-0"/><span className="flex-1">{recommendationError.includes('GEMINI_HIGH_DEMAND')?'Gemini est temporairement surchargé, réessaie dans quelques instants':recommendationError} — les recommandations locales restent disponibles.</span><button onClick={()=>void runRecommendations()} className="px-3 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white font-bold">Réessayer</button></div>}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">{[['Trades analysés',String(stats.trades),'text-white'],['Pertes potentielles estimées',`-${displayedRecommendations.reduce((s,r)=>s+r.estimatedCost,0).toFixed(0)} ${currency}`,'text-rose-400'],['Gain potentiel estimé',`+${displayedRecommendations.reduce((s,r)=>s+r.impact,0).toFixed(0)} ${currency}/mois`,'text-emerald-400'],['Domaines d’amélioration',String(displayedRecommendations.filter(r=>r.status!=='Résolu').length),'text-white'],['Score d’optimisation',`${optimizationScore}/100`,'text-fuchsia-300']].map(([a,b,c])=><div key={a} className={`${card} p-4`}><div className="text-[9px] uppercase tracking-wider text-slate-500 font-bold">{a}</div><div className={`text-sm font-black mt-2 ${c}`}>{b}</div></div>)}</div>
      {loadingRecommendations&&<div className="text-[11px] text-slate-500 flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin text-fuchsia-400"/>Gemini enrichit les recommandations locales…</div>}
      {displayedRecommendations.length>0&&<div className="space-y-4">{displayedRecommendations.map(r=><RecommendationCard key={r.id} rec={r} onStatus={updateStatus} onApply={applyPlan} onView={setSelectedRecommendation}/>)}</div>}
      {!displayedRecommendations.length&&!loadingRecommendations&&<div className={`${card} p-8 text-center`}><CheckCircle2 className="w-8 h-8 mx-auto text-emerald-400 mb-2"/><div className="text-sm font-bold text-white">Aucun pattern préoccupant détecté</div><p className="text-xs text-slate-500 mt-1">Le système d’alertes local n’a identifié aucun axe prioritaire. Les indicateurs ci-dessus restent calculés à partir de vos données réelles.</p></div>}
    </div>}
    {selectedRecommendation&&<div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setSelectedRecommendation(null)}><div className={`${card} max-w-xl w-full p-6`} onClick={e=>e.stopPropagation()}><div className="flex items-center justify-between"><div><div className="text-[9px] uppercase tracking-wider text-fuchsia-300 font-black">Plan d'action</div><h2 className="text-lg font-black text-white mt-1">{selectedRecommendation.title}</h2></div><button onClick={()=>setSelectedRecommendation(null)} className="text-slate-500 hover:text-white">×</button></div><p className="text-sm text-slate-300 leading-6 mt-4">{selectedRecommendation.description}</p><ul className="mt-4 space-y-2">{selectedRecommendation.quickFixes.map((f,i)=><li key={i} className="text-xs text-slate-300 flex gap-2"><CheckCircle2 className="w-4 h-4 text-violet-400 shrink-0"/>{f}</li>)}</ul></div></div>}
  </div>;
};
