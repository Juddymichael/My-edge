...
export const MyEdgeView: React.FC<MyEdgeViewProps> = ({
  ...
  const [openFilter, setOpenFilter] = useState<'period'|'symbol'|'setup'|'killzone'|'direction'|'result'|null>(null);
        ...
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="lg:w-[245px] shrink-0 space-y-1.5">
            ...
            {([
              ...
              const hasValue = ({period:selectedPeriod,symbol:selectedSymbol,setup:selectedSetup,killzone:selectedKillzone,direction:selectedDirection,result:selectedResult} as Record<string,string>)[key] !== 'ALL';
          ...
          <div className="flex-1 min-w-0 min-h-[70px]">
            <div className={`edge-filter-panel h-full rounded-2xl border border-slate-200 dark:border-[#292E38] bg-slate-50 dark:bg-[#0B0D12] overflow-hidden ${openFilter ? 'is-open' : ''}`}>
              {!openFilter ? <div className="h-full min-h-[70px] flex items-center justify-center text-xs text-slate-400 dark:text-slate-500">Sélectionnez un filtre à gauche pour afficher ses options</div> : <div className="p-4 animate-filter-panel">
                ...
                {openFilter==='killzone'&&<div><p className="text-[10px] font-bold uppercase tracking-wider text-[var(--edge-accent)] mb-2">Killzone</p><div className="flex flex-wrap gap-2">{[['ALL','Toutes les killzones'],['LONDON','Killzone Londres'],['NEW_YORK','New York'],['TOKYO','Tokyo / Asie'],['Killzone Sydney','Sydney']].map(([v,l])=><button key={v} onClick={()=>setSelectedKillzone(v)} className={`px-3 py-2 rounded-xl border text-xs font-bold btn-press ${selectedKillzone===v?'bg-[var(--edge-accent)] text-white border-[var(--edge-accent)]':'bg-white dark:bg-[#181C25] border-slate-200 dark:border-[#292E38] text-slate-700 dark:text-slate-200'}`}>{l}</button>)}</div></div>}
